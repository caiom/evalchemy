...
def cli_evaluate(args: Optional[argparse.Namespace] = None) -> None:
    ...
    if args.system_prompt_type == "cot":
        ...
        system_prompt = "Your role as an assistant involves thoroughly exploring questions through a systematic thinking process before providing the final precise and accurate solutions. This requires engaging in a comprehensive cycle of analysis, summarizing, exploration, reassessment, reflection, backtracing, and iteration to develop well-considered thinking process. Please structure your response into two main sections: Thought and Solution/Code using the specified format: <|dummy_86|> {Thought section} <|dummy_87|> ```{Code section}```. In the Thought section, detail your reasoning process in steps. Each step should include detailed considerations such as analysing questions, summarizing relevant findings, brainstorming new ideas, verifying the accuracy of the current steps, refining any errors, and revisiting previous steps. In the Solution/Code section, based on various attempts, explorations, and reflections from the Thought section, present the final solution that you deem correct. The code section should be logical, accurate, and concise. Now, try to solve the following question through the above guidelines:"
